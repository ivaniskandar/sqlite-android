...
// A templated helper function that calls the right strerror_r.
template <typename T>
static inline const char* strerror_r_helper(T strerror_r_ptr, int errnum, char* buf, size_t buflen) {
    // This overload will be chosen for the int-returning POSIX version.
    if (strerror_r_ptr(errnum, buf, buflen) != 0) {
        snprintf(buf, buflen, "errno %d", errnum);
    }
    return buf;
}

static inline const char* strerror_r_helper(char* (*strerror_r_ptr)(int, char*, size_t), int errnum, char* buf, size_t buflen) {
    // This overload will be chosen for the char*-returning GNU version.
    return strerror_r_ptr(errnum, buf, buflen);
}

const char* jniStrError(int errnum, char* buf, size_t buflen) {
    return strerror_r_helper(&strerror_r, errnum, buf, buflen);
}
